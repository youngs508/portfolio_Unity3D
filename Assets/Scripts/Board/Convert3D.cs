...
public class Convert3D : MonoBehaviour
{
    ...
    private bool _isAllMadeToggle = false;  // 모든 블럭이 매칭됐을때 update함수가 계속 실행되는 것을 막을때 사용
    ...
    public void BombBlock()
    {
        ...
        foreach (GameObject obj in _board)
        {
            if (obj != null)
            {
                ...
                //obj.GetComponent<BoxCollider>().size = new Vector3(1.2f, 1.2f, 1.2f);
    ...
    private bool IsAllBlockMade()
    {
        ...
        return true;    // 보드상에 모든 블럭이 Fixed 된 경우
    }
    ...
    /// <summary>
    /// 보드상에 선택된 블럭의 컬러에 해당 하는 블럭을 모두 찾아서 
    /// 백보드에 위치시킨다.
    /// </summary>
    ...
    private void AllBlockFixedPosition(int index)
    {
        ...
        // 백보드상에 index와 동일한 블럭을 찾아서 matchIndexBlockList 저장한다.
        for(int i = 0; i < _currentColumn; i++)
        {
            for(int j = 0; j < _currentRow; j++)
            {
